...
SkewedSprite.onSkew = Method.PUBLIC(function (Self)
    for i = 1, #Self.skewedSprites do
        ...
        HScript.call('FlxG.cameras.lock')

        HScript.call('draw', skewed, skewed.skew, skewed._skewMatrix, skewed.exposedMatrix, skewed.transformMatrix, skewed.useVertices, skewed.vertices)
        HScript.call('FlxG.plugins.draw')
        HScript.call('FlxG.state.draw')

        HScript.call('FlxG.cameras.render')
        HScript.call('FlxG.cameras.unlock')
    end
end, true)
...
if not Signals.postDraw.has(SkewedSprite.onSkew) then
    Object.waitingList.add(function()
        ...
        HScript.execute [[
            function updateSkewMatrix(skewMatrix, skewPoint, useVertices, vertices){
                skewMatrix.identity();
            ...
            function drawComplex(camera, sprite, skewPoint, skewMatrix, exposedMatrix, transformMatrix, useVertices, vertices){
                ...
                if (exposedMatrix){
                    ...
                    if (sprite.bakedRotationAngle <= 0){
                        ...
                        updateSkewMatrix(skewMatrix, skewPoint, useVertices, vertices);
            ...
            function simpleRendererCheck(camera, sprite, skewPoint, matrixExposed){
                if (FlxG.renderBlit)
                {
            ...
            function draw(sprite, skewPoint, skewMatrix, exposedMatrix, transformMatrix, useVertices, vertices){
                ...
                for (camera in sprite.cameras)
                {
                	...
                	if (simpleRendererCheck(camera, sprite, skewPoint, exposedMatrix)){    
                		sprite.drawSimple(camera);
                	}else{
                		drawComplex(camera, sprite, skewPoint, skewMatrix, exposedMatrix, transformMatrix, useVertices, vertices);
                	}
                }
            }
        ]]
